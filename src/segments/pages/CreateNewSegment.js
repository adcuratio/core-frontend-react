...
import SegmentFilterTabs from '../components/segment-filter-tabs';
...
import UploadFirstPartyData from '../components/upload-first-party-data';
import ExperianFilterContent from '../components/experian-filter-content/experianFilterContent';
...
import { formatText, showAckMessage, showAckErrorMessage, hasProperty, validateName } from '../../common/utils';
...
@inject('segmentStore')
@observer
class CreateNewSegment extends React.Component {
  constructor(props) {
    super(props);
    //const liteFilters = toJS(props.segmentStore.liteFilters) || []; get all the filer for lite group
    //const completeFilters = toJS(props.segmentStore.fullFilters) || [];  get all the filters for the complete group
    this.state = {
  ...
  componentDidMount = () => {
    ...
    this._isMounted &&
      ...
      this.props.segmentStore.getAllTspFullGroupCompanies().then((res) => {
        this.setState({ ...this.state, completeFilters: res, isPageLoading: false });
      });
    this._isMounted &&
  ...
  handleSelectAllOrNoneFunc = (id, title, type) => {
    ...
    if (activeFilterTab.id === 'quick_filters') {
      ...
      if (id === 'credit') {
        ...
        if (type === 'all') {
          completeFilters[indexOfCredit].template.sub_cat[indexOfFilter].filters.map(
        ...
        if (type === 'none') {
          completeFilters[index].template.filters.map((obj) => (obj.isTabSelected = false));
        } else if (type === 'all') {
  ...
  handleUserInputFuncForExperian = ({ selectedValue, title, filtersForExperian, activeSubCategoryData, type }) => {
    ...
    if (type === 'experian_quick') {
      ...
      const indexOfFilter = experianFilters[indexOfCat].template.filters.indexOf(title);
      if (experianFilters[indexOfCat].template.filters[indexOfFilter].field_type === 'multi-select-dropdown') {
        if (title.filter_name === 'zip_code') {
          if (experianFilters[indexOfCat].template.filters[indexOfFilter].value.includes(selectedValue)) {
            const newArr = experianFilters[indexOfCat].template.filters[indexOfFilter].value.filter(
              (item) => item !== selectedValue
            );
            delete experianFilters[indexOfCat].template.filters[indexOfFilter].value;
            experianFilters[indexOfCat].template.filters[indexOfFilter].value = newArr;
          } else {
            if (experianFilters[indexOfCat].template.filters[indexOfFilter].value === '') {
              const arr = [selectedValue];
              experianFilters[indexOfCat].template.filters[indexOfFilter].value = arr;
            } else {
              experianFilters[indexOfCat].template.filters[indexOfFilter].value.push(selectedValue);
            }
          }
        } else {
          if (experianFilters[indexOfCat].template.filters[indexOfFilter].value === '') {
            experianFilters[indexOfCat].template.filters[indexOfFilter].value = selectedValue;
          } else {
            delete experianFilters[indexOfCat].template.filters[indexOfFilter].value;
            experianFilters[indexOfCat].template.filters[indexOfFilter].value = selectedValue;
          }
        ...
        experianFilters[indexOfCat].template.filters[indexOfFilter].filter_name === 'mosaic_global_household'
      ) {
        const valString = [];
        experianFilters[indexOfCat].template.filters[indexOfFilter].options.forEach((item) => {
        ...
        experianFilters[indexOfCat].template.filters[indexOfFilter].value = valString;
      } else {
        experianFilters[indexOfCat].template.filters[indexOfFilter].value = selectedValue;
        if (hasProperty(experianFilters[indexOfCat].template.filters[indexOfFilter], 'isValue')) {
      ...
      const indexOfSubCat = experianFilters[indexOfCat].template.sub_cat.indexOf(filtersForExperian);
      const indexOfFilter = experianFilters[indexOfCat].template.sub_cat[indexOfSubCat].filters.indexOf(title);

      experianFilters[indexOfCat].template.sub_cat[indexOfSubCat].filters[indexOfFilter].value = selectedValue;
      if (hasProperty(title, 'isValue')) {
        experianFilters[indexOfCat].template.sub_cat[indexOfSubCat].filters[indexOfFilter].isValue =
          !experianFilters[indexOfCat].template.sub_cat[indexOfSubCat].filters[indexOfFilter].isValue;
        experianFilters[indexOfCat].template.sub_cat[indexOfSubCat].filters[indexOfFilter].isValue
          ? (experianFilters[indexOfCat].template.sub_cat[indexOfSubCat].filters[indexOfFilter].value = selectedValue)
          : (experianFilters[indexOfCat].template.sub_cat[indexOfSubCat].filters[indexOfFilter].value = '');
      }
      if (title.title === 'Age') {
      ...
      const indexOfCat = experianFilters[indexOfAuto].template.sub_cat.indexOf(activeSubCategoryData);
      const indexOfSubSubCategory =
        experianFilters[indexOfAuto].template.sub_cat[indexOfCat].sub_cat.indexOf(filtersForExperian);
      const indexOfFilter =
        experianFilters[indexOfAuto].template.sub_cat[indexOfCat].sub_cat[indexOfSubSubCategory].filters.indexOf(title);
      experianFilters[indexOfAuto].template.sub_cat[indexOfCat].sub_cat[indexOfSubSubCategory].filters[
        indexOfFilter
      ].value = selectedValue;
    }
  ...
  handleUserInputFunc = (values, id) => {
    ...
    if (activeFilterTab.id === 'quick_filters') {
      ...
      if (
        ...
        if (values[2] === 'Yes' || values[2] === 'No' || values[2] === '') {
          ...
          if (temp[index].template.filters[filterIndex].value === 'Any') {
            ...
            temp[index].template.filters[filterIndex].comboFilters.forEach((combo) => {
              const indexofCombo = temp[index].template.filters[filterIndex].comboFilters.indexOf(combo);
              combo.forEach((comboFil) => {
                if (comboFil.filter_name.slice(0, -2) === 'vehicle_make' && comboFil.filter_name === values[2][0]) {
                  const model = [];
                  this.props.segmentStore.getCorrespondingModelYear(values[2][1]).then((data) => {
                    if (data && data.model_year?.length !== 0) {
                      data.model_year.forEach((modelArr) => model.push(modelArr.model));
                      const modalYear = [{ values: model }];
                      const modelNew = { ...temp[index].template.filters[filterIndex].comboFilters[indexofCombo][1] };
                      delete temp[index].template.filters[filterIndex].comboFilters[indexofCombo][1];
                      temp[index].template.filters[filterIndex].comboFilters[indexofCombo][1] = {
                        ...modelNew,
                        options: modalYear,
                        value: '',
                        model_year_data: data,
                      };
                      temp[index].template.filters[filterIndex].comboFilters[indexofCombo][2].value = '';
                    } else {
                      temp[index].template.filters[filterIndex].comboFilters[indexofCombo][1] = {
                        ...temp[index].template.filters[filterIndex].comboFilters[indexofCombo][1],
                        options: [{ values: [] }],
                  ...
                  temp[index].template.filters[filterIndex].comboFilters[
                    ...
                    if (model.model === values[2][1]) {
                      ...
                      const yearNew = { ...temp[index].template.filters[filterIndex].comboFilters[indexofCombo][2] };
                      delete temp[index].template.filters[filterIndex].comboFilters[indexofCombo][2];
                      temp[index].template.filters[filterIndex].comboFilters[indexofCombo][2] = {
                        ...yearNew,
                        options: modalYear,
                        value: '',
                      };
      ...
      if (id === 'credit') {
        ...
        temp[indexOfCredit].template.sub_cat[index].filters[filterIndex].options[optionIndex].isValueSelected =
          !temp[indexOfCredit].template.sub_cat[index].filters[filterIndex].options[optionIndex].isValueSelected;
  ...
  handleSubCategoryTitleTabChange = (parentData, parentName, selectedData) => {
    ...
    if (parentName !== 'auto_sub') {
      ...
      experianCpy[indexOfParent].template.sub_cat.forEach((sub) => {
        if (experianCpy[indexOfParent].template.sub_cat.indexOf(sub) === indexOfChild) {
      ...
      const indexOfSubCategory = autoFilter[0].template.sub_cat.indexOf(parentData);
      const indexOfSubSubCategory = autoFilter[0].template.sub_cat[indexOfSubCategory].sub_cat.indexOf(selectedData);
      experianCpy[indexOfAuto].template.sub_cat[indexOfSubCategory].sub_cat.forEach((sub) => {
        if (
          experianCpy[indexOfAuto].template.sub_cat[indexOfSubCategory].sub_cat.indexOf(sub) === indexOfSubSubCategory
        ) {
  ...
  handleAddRemoveCategoryForExperianFunc = (data) => {
    ...
    if (experianFilters.find((data) => data.isSelected) && !flag) {
      const activeLiteIndex = experianFilters.indexOf(experianFilters.find((data) => data.isSelected));
  ...
  handleTabChangeFunctionsForExperian = (title, activeData, id = '', activeSubCategoryData) => {
    const { experianFilters } = this.state;
    if (id === 'experian_non_auto') {
      const subCategoryIndex = experianFilters.indexOf(activeSubCategoryData);
      const indexOfSubCat = experianFilters[subCategoryIndex].template.sub_cat.indexOf(title);
      const indexOfFilter =
        experianFilters[subCategoryIndex].template.sub_cat[indexOfSubCat].filters.indexOf(activeData);

      experianFilters[subCategoryIndex].template.sub_cat[indexOfSubCat].filters[indexOfFilter].isFilterSelected =
        !experianFilters[subCategoryIndex].template.sub_cat[indexOfSubCat].filters[indexOfFilter].isFilterSelected;
    } else if (id === 'experian_quick') {
      const indexOfCat = experianFilters.indexOf(title);
      const indexOfFilter = experianFilters[indexOfCat].template.filters.indexOf(activeData);
      experianFilters[indexOfCat].template.filters[indexOfFilter].isFilterSelected =
        !experianFilters[indexOfCat].template.filters[indexOfFilter].isFilterSelected;
    }
    this.setState({ ...this.state, experianFilters });
  };
  ...
  handleAutoTabFilters = ({ activeSubCategoryData, filtersForExperian, data }) => {
    ...
    const indexOfAutoSubCategory = experianFilters[indexOfAuto].template.sub_cat.indexOf(activeSubCategoryData);
    const indexOfSubSubCategory =
      experianFilters[indexOfAuto].template.sub_cat[indexOfAutoSubCategory].sub_cat.indexOf(filtersForExperian);
    experianFilters[indexOfAuto].template.sub_cat[indexOfAutoSubCategory].sub_cat[
  ...
  handleSelectAllOrNoneFiltersForExperian = (id, title, type, subCatData) => {
    ...
    if (id === 'experian_non_auto') {
      ...
      const indexOfSubCat = experianFilters[indexOfCategory].template.sub_cat.indexOf(title);
      if (type === 'all') {
        experianFilters[indexOfCategory].template.sub_cat[indexOfSubCat].filters.forEach((fill) => {
      ...
      const indexOfCat = experianFilters.indexOf(title);
      type === 'all'
        ? experianFilters[indexOfCat].template.filters.forEach((fil) => (fil.isFilterSelected = true))
        : experianFilters[indexOfCat].template.filters.forEach((fil) => {
            fil.isFilterSelected = false;
            fil.value = '';
          });
    } else if (id === 'experian_auto') {
      const autoData = experianFilters.filter((exp) => exp.name === 'Auto');
      const indexOfAuto = experianFilters.indexOf(autoData[0]);
      const indexOfCat = experianFilters[indexOfAuto].template.sub_cat.indexOf(subCatData);
      const indexOfSubSubCategory = experianFilters[indexOfAuto].template.sub_cat[indexOfCat].sub_cat.indexOf(title);
      type === 'all'
        ? experianFilters[indexOfAuto].template.sub_cat[indexOfCat].sub_cat[indexOfSubSubCategory].filters.forEach(
  ...
  handleTabChangeFunc = (title, activeData, id = '') => {
    ...
    if (activeFilterTab.id === 'quick_filters') {
      ...
      if (id === 'credit') {
        ...
        temp[indexOfCredit].template.sub_cat[index].filters[filterIndex].isFilterSelected =
          !temp[indexOfCredit].template.sub_cat[index].filters[filterIndex].isFilterSelected;
        this.setState({ ...this.state, completeFilters: temp });
      } else {
  ...
  saveAPIcall = (temp = null, tempTitle = null) => {
    ...
    if (activeFilterTab.value === 0 || activeFilterTab.value === 1 || activeFilterTab.value === 3) {
      payload = {
        description: formatText(this.state.segmentDescription),
        filter_json: temp,
        filter_title: tempTitle,
        is_stb: false,
        is_wanted: true,
      ...
      } else if (this.props.$state.params.selectedNodeDataType === 'sub_brand') {
    ...
    this.props.segmentStore.postUserDataForSegmentCreation(payload).then((res) => {
      if (res === undefined) {
        showAckErrorMessage({ message: 'Something went wrong while saving Segment!' });
      }
      //POST request
      if (res.data.success) {
        const message = 'Segment created successfully';
        showAckMessage({ message });
        this.props.navigationService.goToSegmentLandingPage(
          this.props.$state.params.selectedNodeData,
          this.props.$state.params.selectedCompany,
          this.props.$state.params.selectedCompanyName
        );
      } else {
    ...
    if (activeFilterTab.value === 0) {
      ...
      return (
        <ExperianFilterContent
          ...
          handleAddRemoveCategoryForExperianFunc={this.handleAddRemoveCategoryForExperianFunc}
          handleSelectAllOrNoneForExperianFunc={this.handleSelectAllOrNoneForExperianFunc}
          handleTabChangeFunctionsForExperian={this.handleTabChangeFunctionsForExperian}
          handleSelectAllOrNoneFiltersForExperian={this.handleSelectAllOrNoneFiltersForExperian}
  ...
  onSaveSegment = () => {
    ...
    if (activeFilterTab.value === 0) {
      ...
      if (flagForCombo === 1) {
        const message = 'Please select all values of combination filter before saving!';
        showAckErrorMessage({ message });
        return;
      }
      if (flag === 2) {
      ...
      if (flag === 3) {
        const message = 'please select both values of individual age before saving!';
        showAckErrorMessage({ message });
        return;
      }
      if (flag === 1) {
      ...
      if (!validateName(this.state.segmentName)) {
        showAckErrorMessage({ message: 'Segment name must be valid.' });
        return;
      }
      ...
      this.state.liteFilters.forEach((obj) => {
        ...
        obj.template.filters.forEach((filter) => {
          if (filter.isTabSelected && (filter.title === 'Annual Income' || filter.title === 'Estimated Net worth')) {
            ...
            tempTitle1[filter.title] = elements;
          } else if (
            filter.value &&
            ...
            tempTitle1[filter.title] = filter.value;
      ...
      this.saveAPIcall(temp, tempTitle);
    }
    if (activeFilterTab.value === 1) {
        ...
        if (obj.name === 'Credit' && obj.isSelected) {
          obj.template.sub_cat.forEach((filters) => {
            ...
            if (Object.keys(temp2).length !== 0) {
              ...
              tempTitle1[filters.title] = temp2;
            }
          });
          if (Object.keys(temp1).length !== 0) {
            temp[obj.name] = temp1;
      ...
      experianFilters.forEach((exp) => {
        if (hasProperty(exp.template, 'sub_cat')) {
          ...
          if (Object.keys(SubSubObj).length !== 0) {
            ...
            tempObjTitle[exp.name] = SubSubObj;
          }
        }
      });
      ...
      this.saveAPIcall(tempObj, tempObjTitle);
    }
  ...
  render() {
    const { activeFilterTab, liteFilters, completeFilters, experianFilters } = this.state;
    return this.props.$state.params.companyId && this.state.liteFilters && this.state.completeFilters ? (
      <MainContent>
        <PageContent className="segment-page-container-height">
          <Row>
            <Col md={9}>
              ...
              />
              <div>{liteFilters.length > 0 && completeFilters.length > 0 && this.segmentTabSwitch()}</div>
            </Col>
            <Col md={3}>
              <SegmentSummary
        ...
        <Loader isLoading={this.state.isPageLoading || this.props.uiStore.isLoading} />
      </MainContent>
    ) : (
...
export default withStore(CreateNewSegment);
