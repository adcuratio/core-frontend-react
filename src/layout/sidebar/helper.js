...
export const superAdminMenuList = ({ $state, navigationService }) => {
  ...
  const isDataProviderSelection = () => $state.current.name === 'dash.superAdminDataProvider';

  const goToMsgVisualization = () => {
    navigationService.goToMsgVisualization();
  };
  const onMsgVisualization = () => $state.current.name === 'dash.msgVisualization';

  const goToViewSegmentsCount = () => {
    navigationService.goToViewSegmentsCount();
  };
  const onViewSegmentsCount = () => $state.current.name === 'dash.viewSegmentsCount';
  ...
  const isSuperAdminDealLogs = () => $state.current.name === 'dash.superAdminDealLogs';

  const goToSuperAdminNetworkLogs = () => {
    navigationService.goToSuperAdminNetworkLogs();
  };
  const isSuperAdminNetworkLogs = () => $state.current.name === 'dash.superAdminNetworkLogs';

  const goToSuperAdminCreativeLogs = () => {
    navigationService.goToSuperAdminCreativeLogs();
  };
  const isSuperAdminCreativeLogs = () => $state.current.name === 'dash.superAdminCreativeLogs';

  const goToSuperAdminCampaignLogs = () => {
    navigationService.goToSuperAdminCampaignLogs();
  };
  const isSuperAdminCampaignLogs = () => $state.current.name === 'dash.superAdminCampaignLogs';

  const goToSuperAdminSegmentsLogs = () => {
    navigationService.goToSuperAdminSegmentsLogs();
  };
  const isSuperAdminSegmentsLogs = () => $state.current.name === 'dash.superAdminSegmentsLogs';

  const goToSuperAdminMGLogs = () => {
    navigationService.goToSuperAdminMGLogs();
  };
  const isSuperAdminMGLogs = () => $state.current.name === 'dash.superAdminMGLogs';

  const goToSuperAdminAttributeIDLogs = () => {
    navigationService.goToSuperAdminAttributeIDLogs();
  };
  const isSuperAdminAttributeIDLogs = () => $state.current.name === 'dash.superAdminAttributeIDLogs';

  const goToSuperAdminUpgradedAdspots = () => {
    navigationService.goToSuperAdminUpgradedAdspots();
  };
  const onSuperAdminUpgradedAdspots = () => $state.current.name === 'dash.superAdminUpgradedAdspots';

  const goToSuperAdminEngineFeedbackSpot = () => {
    navigationService.goToSuperAdminEngineFeedbackSpot();
  };
  const onEngineFeedbackSpot = () => $state.current.name === 'dash.superAdminEngineFeedbackSpot';
  ...
  const onSuperAdminXanDeliveryRepView = () => $state.current.name === 'dash.superAdminXanDeliveryRepView';

  const goToSuperAdminManageCampaigns = () => {
    navigationService.goToSuperAdminManageCampaigns();
  };
  const onSuperAdminManageCampaigns = () => $state.current.name === 'dash.superAdminManageCampaigns';

  const goToSuperAdminRecurringSchedule = () => {
    navigationService.goToSuperAdminRecurringSchedule();
  };
  const onSuperAdminRecurringSchedule = () => $state.current.name === 'dash.superAdminRecurringSchedule';

  const isCreativesModule = () => isSuperAdminManageCreatives() || isSuperAdminCreativesWatermarkApproval();

  const goToSuperAdminManageCreatives = () => {
    navigationService.goToSuperAdminManageCreatives();
  };
  const isSuperAdminManageCreatives = () => $state.current.name === 'dash.superAdminManageCreatives';
  ...
  const onSuperAdminPacingReports = () => $state.current.name === 'dash.superAdminPacingReports';
  ...
  if (process.env.REACT_APP_BACKEND === 'https://production-api.adcuratio.net') {
...
export const advertiserAdminMenuList = ({ $state, navigationService }) => {
  ...
  const isBrandSubBrand = () =>
    $state.current.name === 'dash.agencyBrandSubBrand' || $state.current.name === 'dash.agencyAdminAddBrand';
...
export const networkAdminMenuList = ({ $state, navigationService }) => {
  ...
  const onAddTraffickingPlan = () => $state.current.name === 'dash.addTraffickingPlan';

  const goToNetworkEdiTable = () => {
    navigationService.goToNetworkEdiTable();
  };
  const isNetworkEdiTable = () => $state.current.name === 'dash.NetworkEdiTable';

  const goToNetworkCreatives = () => {
    navigationService.goToNetworkCreatives();
  };
  const onNetworkCreatives = () => $state.current.name === 'dash.networkCreatives';

  const goToNetworkUpgradedAdspots = () => {
    navigationService.goToNetworkUpgradedAdspots();
  };
  const onNetworkUpgradedAdspots = () => $state.current.name === 'dash.networkUpgradedAdspots';

  const goToNetworkPostLogs = () => {
    navigationService.goToNetworkPostLogs();
  };
  const onNetworkPostLogs = () => $state.current.name === 'dash.networkPostLogs';
...
export const vizioFoxAdminMenuList = ({ $state, navigationService }) => {
  ...
  const onFoxImpressionData = () => $state.current.name === 'dash.onFoxImpressionData';
...
export const operatorAdminUserMenuList = ({ $state, navigationService }) => {
  ...
  const isNcmCampaignOperator = () => $state.current.name === 'dash.ncmCampaignOperator';

  const goToOperatorCreatives = () => {
    navigationService.goToOperatorCreatives();
  };
  const isOperatorCreatives = () => $state.current.name === 'dash.operatorCreatives';
  ...
  const isAggregateInventory = () => $state.current.name === 'dash.aggregateInventory';

  const goToOperatorPostLogs = () => {
    navigationService.goToOperatorPostLogs();
  };
  const onOperatorPostLogs = () => $state.current.name === 'dash.operatorPostLogs';
  const goToManageDishAudience = () => {
    navigationService.goToManageDishAudience();
  };
  const isManageDishAudience = () => $state.current.name === 'dash.manageDishAudience';
...
export const vizioMenuList = ({ $state, navigationService }) => {
  ...
  const onVizioManageOrders = () => $state.current.name === 'dash.vizioManageOrders';

  const goToVizioTargetSegments = () => {
    navigationService.goToVizioTargetSegments();
  };
  const onVizioTargetSegments = () => $state.current.name === 'dash.vizioTargetSegments';

  const isCreativesModule = () => isVizioReviewCreatives() || isVizioCreativesWatermarkApproval();

  const goToVizioReviewCreatives = () => {
    navigationService.goToVizioReviewCreatives();
  };
  const isVizioReviewCreatives = () => $state.current.name === 'dash.vizioReviewCreatives';

  const goToVizioCreativesWatermarkApproval = () => {
    navigationService.goToVizioCreativesWatermarkApproval();
  };
  const isVizioCreativesWatermarkApproval = () => $state.current.name === 'dash.vizioCreativesWatermarkApproval';
...
export const xandrMenuList = ({ $state, navigationService }) => {
  ...
  const onXandrSegmentFiles = () => $state.current.name === 'dash.xandrSegmentFiles';

  const goToXandrManageOrders = () => {
    navigationService.goToXandrManageOrders();
  };
  const onXandrManageOrders = () => $state.current.name === 'dash.xandrManageOrders';
...
export const cbsNetworkAdminMenuList = ({ $state, navigationService }) => {
  ...
  const onCbsNetworkLanding = () => $state.current.name === 'dash.cbsNetworkLanding';

  const goToCbsAddTraffickingPlan = () => {
    navigationService.goToCbsAddTraffickingPlan();
  };
  const onCbsAddTraffickingPlan = () => $state.current.name === 'dash.cbsAddTraffickingPlan';

  const goToCbsNetworkEdiTable = () => {
    navigationService.goToCbsNetworkEdiTable();
  };
  const onCbsNetworkEdiTable = () => $state.current.name === 'dash.cbsNetworkEdiTable';

  const goToCbsManageChannels = () => {
    navigationService.goToCbsManageChannels();
  };
  const onCbsManageChannels = () => $state.current.name === 'dash.cbsManageChannels';

  const isCreativesModule = () => isCbsManageCreatives() || isCbsReviewCreatives() || isCbsCreativesWatermarkApproval();

  const goToCbsManageCreatives = () => {
    navigationService.goToCbsManageCreatives();
  };
  const isCbsManageCreatives = () => $state.current.name === 'dash.cbsManageCreatives';

  const goToCbsReviewCreatives = () => {
    navigationService.goToCbsReviewCreatives();
  };
  const isCbsReviewCreatives = () => $state.current.name === 'dash.cbsReviewCreatives';

  const goToCbsCreativesWatermarkApproval = () => {
    navigationService.goToCbsCreativesWatermarkApproval();
  };
  const isCbsCreativesWatermarkApproval = () => $state.current.name === 'dash.cbsCreativesWatermarkApproval';
...
export const foxNetworkAdminMenuList = ({ $state, navigationService }) => {
  ...
  const onFoxNetworkLanding = () => $state.current.name === 'dash.foxNetworkLanding';

  const goToFoxAddTraffickingPlan = () => {
    navigationService.goToFoxAddTraffickingPlan();
  };
  const onFoxAddTraffickingPlan = () => $state.current.name === 'dash.foxAddTraffickingPlan';

  const goToFoxNetworkEdiTable = () => {
    navigationService.goToFoxNetworkEdiTable();
  };
  const onFoxNetworkEdiTable = () => $state.current.name === 'dash.foxNetworkEdiTable';

  const goToFoxManageChannels = () => {
    navigationService.goToFoxManageChannels();
  };
  const onFoxManageChannels = () => $state.current.name === 'dash.foxManageChannels';

  // navigation to fox delivery reports
  const goToFoxDeliveryReports = () => {
    navigationService.goToFoxDeliveryReports();
  };
  const isFoxDeliveryReports = () => $state.current.name === 'dash.foxDeliveryReports';

  const isCreativesModule = () => isFoxManageCreatives() || isFoxReviewCreatives() || isFoxCreativesWatermarkApproval();

  const goToFoxManageCreatives = () => {
    navigationService.goToFoxManageCreatives();
  };
  const isFoxManageCreatives = () => $state.current.name === 'dash.foxManageCreatives';

  const goToFoxReviewCreatives = () => {
    navigationService.goToFoxReviewCreatives();
  };
  const isFoxReviewCreatives = () => $state.current.name === 'dash.foxReviewCreatives';

  const goToFoxCreativesWatermarkApproval = () => {
    navigationService.goToFoxCreativesWatermarkApproval();
  };
  const isFoxCreativesWatermarkApproval = () => $state.current.name === 'dash.foxCreativesWatermarkApproval';
...
export const univisionNetworkAdminMenuList = ({ $state, navigationService }) => {
  ...
  const goToManageAudienceLanding = () => {
    navigationService.goToManageAudienceLanding();
  };
  const onManageAudienceLanding = () => $state.current.name === 'dash.manageAudienceLanding';

  const isUnivisionManageAudience = () => onManageAudienceLanding() || isCreateAudience() || onUnivisionViewAudience();

  const goToUnivisionCreateAudience = () => {
    navigationService.goToUnivisionCreateAudience();
  };
  const onUnivisionCreateAudience = () => $state.current.name === 'dash.univisionCreateAudience';

  const onUnivisionSelectFilters = () => $state.current.name === 'dash.univisionSelectAudience';

  const isCreateAudience = () => onUnivisionCreateAudience() || onUnivisionSelectFilters();

  const goToUnivisionViewAudience = () => {
    navigationService.goToUnivisionViewAudience();
  };
  const onUnivisionViewAudience = () => $state.current.name === 'dash.univisionViewAudience';

  const goToUnivisionNetworkLogs = () => {
    navigationService.goToUnivisionNetworkLogs();
  };
  ...
  const onUnivisionNetworkLogs = () => $state.current.name === 'dash.univisionNetworkLogs';
  ...
  const onUnivisionAdministration = () => $state.current.name === 'dash.univisionAdministration';
  ...
  const isUnivisionDeliveryReports = () => $state.current.name === 'dash.univisionDeliveryReports';
  ...
  const goToUnivisionReportsPacing = () => {
    navigationService.goToUnivisionReportsPacing();
  };
  ...
  const isUnivisionReportsPacing = () => $state.current.name === 'dash.univisionReportsPacing';

  const isUnivisionReportsPostCampaign = () => $state.current.name === 'dash.univisionReportsPostCampaign';

  const isUnivisionPostCampaignDaypartReports = () =>
    $state.current.name === 'dash.univisionPostCampaignDaypartReports';

  const isUnivisionReportsCtvLogs = () => $state.current.name === 'dash.univisionReportsCtvLogs';
  ...
  const goToUnivisionInventorySpot = () => {
    navigationService.goToUnivisionInventorySpot();
  };
  // const onUnivisionInventorySpots = () => $state.current.name === 'dash.univisionInventorySpots';

  const onUnivisionInventorySpot = () => $state.current.name === 'dash.univisionInventorySpot';

  const isViewSchedules = () => onUnivisionNetworkLogs() || onUnivisionInventorySpot();

  const goToUnivisionManageCampaigns = () => {
    navigationService.goToUnivisionManageCampaigns();
  };
  const onUnivisionManageCampaigns = () => $state.current.name === 'dash.univisionManageCampaigns';
...
const reportHelper = ({ $state, navigationService }) => {
  ...
  const isCampaignReports = () =>
    $state.current.name === 'dash.xandrCampaignReports' || $state.current.name === 'dash.networkCampaignReports';
  const isBillingReports = () => $state.current.name === 'dash.xandrBillingReports';

  const isBillingTrends = () => $state.current.name === 'dash.xandrBillingTrends';
  const isNetworkDeliveryReports = () => $state.current.name === 'dash.networkDeliveryReports';

  const goToAgencyCampaignReports = () => {
    navigationService.goToAgencyCampaignReports();
  };
  const isAgencyImpressionReports = () => $state.current.name === 'dash.agencyCampaignReports';

  const goToAdvertiserCampaignReports = () => {
    navigationService.goToAdvertiserCampaignReports();
  };
  const isAdvertiserImpressionReports = () => $state.current.name === 'dash.advertiserCampaignReports';

  const goToAgencyDeliveryReports = () => {
    navigationService.goToAgencyDeliveryReports();
  };
  const isAgencyDeliveryReports = () => $state.current.name === 'dash.agencyDeliveryReports';

  const goToAdvertiserDeliveryReports = () => {
    navigationService.goToAdvertiserDeliveryReports();
  };
  const isAdvertiserDeliveryReports = () => $state.current.name === 'dash.advertiserDeliveryReports';
  ...
  const isOprImpressionReports = () => $state.current.name === 'dash.operatorImpressionReports';

  const goToOperatorImpressionReportsSaso = () => {
    navigationService.goToOperatorImpressionReportsSaso();
  };
  const isOprImpressionReportsSaso = () => $state.current.name === 'dash.operatorImpressionReportsSaso';

  const goToOperatorImpressionReportsMaso = () => {
    navigationService.goToOperatorImpressionReportsMaso();
  };
  const isOprImpressionReportsMaso = () => $state.current.name === 'dash.operatorImpressionReportsMaso';

  const goToOperatorImpressionReportsAgg = () => {
    navigationService.goToOperatorImpressionReportsAgg();
  };
  const isOprImpressionReportsAgg = () => $state.current.name === 'dash.operatorImpressionReportsAgg';
  ...
  const isOperatorReportsPacing = () => $state.current.name === 'dash.univisionReportsPacing';

  const isOperatorPostCampaignDaypartReports = () => $state.current.name === 'dash.univisionPostCampaignDaypartReports';

  const isOperatorReportsPostCampaign = () => $state.current.name === 'dash.univisionReportsPostCampaign';
  ...
  const isCbsDeliveryReports = () => $state.current.name === 'dash.cbsDeliveryReports';
...
const commonHelper = ({ $state, navigationService }) => {
  ...
  const isAudienceGroup = () =>
    $state.current.name === 'dash.segmentLandingPage' || $state.current.name === 'dash.createNewSegment';

  const goToMsgGrps = () => {
    navigationService.goToMsgGrps();
  };
  const isMsgGrp = () =>
    $state.current.name === 'dash.messagingGroup' || $state.current.name === 'dash.defineMessagingGroups';
  ...
  const isAgencyAdminManageUsers = () => $state.current.name === 'dash.agencyAdminManageUsers';
  ...
  const isAgencyRepBrandSubBrand = () =>
    $state.current.name === 'dash.agencyBrandSubBrand' || $state.current.name === 'app.agencyAdminAddCompany';
...
export const isAuthorized = function ({ $state, navigationService, authStore }) {
  ...
  } else if (isNetworkAdminUser && authStore.userObj.related_data.network.name === 'vizio_fox') {
  ...
  } else if (isOperatorAdminUser && authStore.userObj.related_data.operator.name === 'dish') {
    return operatorAdminUserMenuList({ $state, navigationService });
  } else if (isOperatorAdminUser && authStore.userObj.related_data.operator.name === 'vizio') {
    return vizioMenuList({ $state, navigationService });
  } else if (isOperatorAdminUser && authStore.userObj.related_data.operator.name === 'xandr') {
