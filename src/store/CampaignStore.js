...
class CampaignStore {
  ...
  async getCMTInventory(deals, entityId, entityType, adId) {
    ...
    try {
      ...
      this.formCMTInventory = res.data.data.map((inv) => ({
        ...inv,
        is_selected: true,
        adspot_date: inv.adspot_date
          ? inv.adspot_date.split("T")[0]
          : "No datetime data",
      }));
      this.uniqAdspotFilterData = {
        uniqNetworks: [
          ...new Set(this.formCMTInventory.map((net) => net.channel_name)),
        ],
        uniqDeals: [
          ...new Set(this.formCMTInventory.map((net) => net.deal_number)),
        ],
        uniqFW: [
          ...new Set(this.formCMTInventory.map((net) => net.adspot_date)),
        ],
        uniqDP: [
          ...new Set(this.formCMTInventory.map((net) => net.day_part_name)),
        ],
  ...
  async getDealIds(entityType, entityId, creativeId) {
    ...
    try {
      ...
      if (res.status === 200) {
        this.formDealIds = res.data.data.map((edi) => ({
          ...edi,
          isSelected: false,
        }));
      }
    ...
    try {
      ...
      if (res.status === 200) {
        this.relatedMSG = {
          ...
          segments: res.data.data.segments.map((inv) => ({
            ...inv,
            isSelected: true,
          })),
          isValid: true,
  ...
  async copySegment(selectedEntity, entityType) {
    ...
    if (entityType === "brand")
      reqString = `${reqString}&brand_id=${selectedEntity.id}`;
    // else {
    ...
    try {
      const res = await API.get(
        `/advertiser/copy_adc_group/?hierarchy_type=${hierarchyType}&${reqString}`
      );
      if (res.status === 200) {
        this.segmentsAvailableForCopy = res.data.map((inv) => ({
          ...inv,
          selected: false,
        }));
  ...
  async getVideoUrl(creativeId) {
    try {
      const res = await API.get(
        `/pilot/get_ftp_creative_files/?item_id=${creativeId}`
      );
      if (res.data.success) {
