...
const ViewPacingReports = inject("authStore")(
  observer((props) => {
    const { authStore, modalData, id } = props;
        ...
        getReports: async () => {
          try {
            ...
            if (id === "graph") {
              ...
              const response = await API.get(
                `admin/get_tableau_report_url_from_pacing/?report_type=${report_type}&user_name=${user_name}&user_type=${user_type}&trade_id=${modalData.id}`
              );
              if (response.data.success) {
                const reportUrlData = response.data.data;
                // parsing tableau_url that will be help to load report
