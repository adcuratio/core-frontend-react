...
const NcmManageTable = (props) => {
  ...
  const tableRow = (a) => (
    ...
    >
      ...
      <td className="mn-font text-align-center-imp">{a.adv_company_name || '---'}</td>
      ...
      <td className="mn-font text-align-center-imp">
        ...
        <div>{`${moment(a?.created).tz('America/New_York').format('YYYY-MM-DD')}`}</div>
        <div>{`${moment(a?.created).tz('America/New_York').format('HH:mm:ss')} EST`}</div>
      </td>

      {activeTab.id === 'cancelled' && (
        <td className="mn-font text-align-center-imp">
          {`${a?.canceled_by?.first_name} ${a?.canceled_by?.last_name}` || '---'}
          <div>{`${moment(a?.canceled).tz('America/New_York').format('YYYY-MM-DD')}`}</div>
          <div>{`${moment(a?.canceled).tz('America/New_York').format('HH:mm:ss')} EST`}</div>
        </td>
      )}

      {(activeTab.id === 'pendingDistributorApproval' || activeTab.id === 'pendingUCIApproval') && (
      ...
      {activeTab.id !== 'pendingUCIApproval' && (
        <td className="mn-font text-align-center-imp">
          {a?.uci_approved
            ? `Confirmed by ${a?.uci_approved?.first_name} ${a?.uci_approved?.last_name} at ${processUTCtoEST(
                a.uci_approved.approved_at
              )} ET`
            : 'N/A'}
          <br />
          {a?.orderline_uci_approval ? null : (
            <OverlayTrigger
              placement="bottom"
              overlay={<Tooltip id="standard-campaign-tooltip">Some orderlines have not been approved</Tooltip>}
            >
              <i
      ...
      <td className="mn-font text-align-center-imp">
        ...
        {activeTab.id !== 'cancelled' && activeTab.id !== 'completed' && activeTab.id !== 'pendingUCIApproval' && (
          <ExtendedCustomBtn
            buttonText="Cancel"
            buttonClassName="tradebtn mn-trade-btn ml5"
      ...
      {activeTab.id === 'pendingUCIApproval' &&
        (a?.created_by?.id !== authStore.getUser()?.related_data?.network_admin_id ? (
          <td className="mn-font text-align-center-imp">
            <ExtendedCustomBtn
              buttonText="Approve"
              buttonClassName="tradebtn mn-trade-btn ml5"
