...
const CampaignDetails = inject("aggCampaignStore")(
  observer((props) => {
    const { afterValidation, selectedCompanyData, levelData, setLevelData } =
      props;
    ...
    const completeStep = (event) => {
      ...
      if (
        !(levelData?.number_of_targeting && levelData?.number_of_targeting >= 1)
      ) {
        showAckErrorMessage({
          message: "Add atleast one orderline to continue.",
        });
        return;
      }
      ...
      for (let i = 0; i < obj?.targeting_data?.length; i++) {
        ...
        if (
          obj?.targeting_data?.[i]?.creative_selection_type === "adid" &&
          !obj?.targeting_data?.[i]?.adid
        ) {
        ...
        if (
          obj?.targeting_data?.[i]?.creative_selection_type ===
          "ad_copy_rotation"
        ) {
          if (
            !(
              ...
              [
                ...
                "Sequenced",
                "Storyboard",
              ].includes(obj?.targeting_data?.[i]?.ad_copy_rotation?.type)
            )
          ) {
          ...
          if (
            !(
              obj?.targeting_data?.[i]?.number_of_creatives &&
              obj?.targeting_data?.[i]?.number_of_creatives > 0
            )
          ) {
          ...
          for (
            let j = 0;
            j < obj?.targeting_data?.[i]?.ad_copy_rotation?.assets?.length;
            j++
          ) {
            if (
              !obj?.targeting_data?.[i]?.ad_copy_rotation?.assets?.[j]?.adid
            ) {
              internalflag = true;
              break;
            }
      ...
      if (errorFlag > 0) {
        showAckErrorMessage({
          message: `Please select ${errorFieldName} in Audience Target ${orderlineNum}`,
        });
        return;
      }
      ...
      obj?.targeting_data?.forEach((target, ind) => {
        if (target?.creative_selection_type === "ad_copy_rotation") {
          if (
            target?.ad_copy_rotation?.type === "Sequenced" ||
            target?.ad_copy_rotation?.type === "Storyboard"
          ) {
            ...
            target?.ad_copy_rotation?.assets?.forEach((asset) => {
              if (Object.prototype.hasOwnProperty.call(asset, "index")) {
                ...
                if (
                  asset?.index < 1 ||
                  asset?.index > target?.number_of_creatives
                ) {
                  index_flag = 1;
                  audienceId = ind + 1;
                  num_creatives = target?.number_of_creatives;
