  ...
  observer((props) => {
          ...
          (res) => {
            if (res.data) {
              if (res.data.success && res.data.data) {
                ...
                showAckErrorMessage({ message: res.data?.message ? res.data?.message : 'No data present on server.' });
            } else showAckErrorMessage({ message: 'No data returned from server.' });
          },
    ...
    const downloadWatermarkedFile = (mData) => {
      if (mData.downloadURL) {
        ...
        downloadLink.download = mData.file_name_mxf ? mData.file_name_mxf : mData.identifier;
        downloadLink.click();
    ...
    const handleDownloadAction = (mData, id) => {
      if (id === 'watermarkFile') {
        ...
        const _creativeData = JSON.parse(JSON.stringify(mData));
        if (mData) {
          creativesStore.getWatermarkedVideoDownloadUrl(mData.original_ad_id).then(
            (res) => {
              if (res.data) {
                if (res.data.success && res.data.data) {
                  _creativeData.downloadURL = res.data.data;
                  downloadWatermarkedFile(_creativeData);
                } else showAckErrorMessage({ message: res.data?.message ?? 'Cannot get video file for the creative.' });
              } else showAckErrorMessage();
            },
            () => showAckErrorMessage({ message: 'Cannot get video file data for the creative.' })
          );
    ...
    const onApproveQA = (actionType, cr_data) => {
      ...
      creativesStore.watermarkQA(payload, cr_data.creative_watermark[0]?.id).then(
        (res) => {
    ...
    const onDeclineQA = (actionType, cr_data) => {
      ...
      const payload = { status: 1, comment: 'Watermark strength is too bright' };
    ...
    return (
      <MainContent>
        ...
        {activeModal === 'qa_approve' || activeModal === 'qa_decline' ? (
          <QAModal
            showModal={activeModal === 'qa_approve' || activeModal === 'qa_decline'}
            closeModal={() => onSetActiveModal('')}
            modalData={modalData}
...
export default withStore(WatermarkCreatives);
