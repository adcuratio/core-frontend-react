...
import { processUTCtoEST, showAckErrorMessage, showAckMessage } from '../../../common/utils';
import { NetworkCreativesTabData, manageHeaderList, declineCodes } from './JsonData';
...
const ReviewCreatives = inject(
  ...
  observer((props) => {
    ...
    const [headerList, setHeaderList] = useState(() => manageHeaderList(NetworkCreativesTabData[0].id));
    ...
    const processCreatives = (res, tabName) => {
      const creativesList = res.data.data.results.map((creative) => {
        ...
        creativeCpy.actual_duration = creative.adid_meta_file_upload?.[0]?.actual_duration.toString() ?? '0';
        ...
        if (tabName === 'approved') {
          ...
          if (
            ...
            creativeCpy?.adid_meta_file_upload?.[0]?.status_list?.channel_daypart_approved_list.length
          ) {
            creativeCpy.channelDaypartApprovedList =
              creativeCpy.adid_meta_file_upload[0].status_list.channel_daypart_approved_list;
            creativeCpy.approvedChannelsAndDayparts = [];

            creativeCpy.adid_meta_file_upload[0].status_list.channel_daypart_approved_list.forEach((stchannel) => {
              ...
              if (dayparts && dayparts.length) {
                obj.dayparts = dayparts
                  .map((d) => {
                    if (d && d.daypart) {
                      const details = `${d.daypart.name} (${getDaypartInfo(d.daypart)})`;
                      return details;
                    }
                    return '';
                  })
        ...
        if (tabName === 'declined') {
          ...
          creativeCpy.declineComment = `${
            creativeCpy.adid_meta_file_upload?.[0]?.status_list?.declined?.[0]?.comment ?? '-'
          }`;
          creativeCpy.declinedAt = creativeCpy.adid_meta_file_upload?.[0]?.status_list?.declined?.[0]?.modified
            ? processUTCtoEST(creativeCpy.adid_meta_file_upload[0].status_list.declined[0].modified)
            : '-';
          creativeCpy.declinedBy = `${
            creativeCpy.adid_meta_file_upload?.[0]?.status_list?.declined?.[0]?.first_name ?? '-'
          } ${creativeCpy.adid_meta_file_upload?.[0]?.status_list?.declined?.[0]?.last_name ?? '-'}`;
        }
      ...
      networkStore.getCreativesForReview(tabName, url).then(
        (response) => {
          if (response.status === 200) {
            ...
            if (url) {
              const creativesListCpy = JSON.parse(JSON.stringify(creativesList));
              const processedCreativesCpy = JSON.parse(JSON.stringify(processedCreatives));
              concatenatedArray = [...creativesListCpy, ...processedCreativesCpy];
            } else {
    ...
    useEffect(() => {
      networkStore.getAllChannels().then(
        (res) => {
          ...
          } else showAckErrorMessage({ message: res?.data?.message ?? 'Unable to fetch Creative List!' });
        },
        () => {
          showAckErrorMessage();
        }
    ...
    const getTabFromTabId = (input_id) => {
      const index = NetworkCreativesTabData.findIndex((tab) => tab?.id === input_id);
      if (index === -1) {
    ...
    const handleModalAction = (buttonType, mData) => {
      if (buttonType === 'viewCreative') {
        getVideoUrl(mData);
      } else if (buttonType === 'approve' || buttonType === 'decline' || buttonType === 'edit') {
        setModalData(mData);
      ...
      <MainContent>
        ...
        {activeModal === 'viewCreative' ? (
          <CreativesAdPreviewModal
            ...
            creativeModalData={creativeModalData && creativeModalData.selectedAdid}
          />
        ) : null}
...
export default withStore(ReviewCreatives);
